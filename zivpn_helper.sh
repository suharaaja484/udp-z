...
function create_manual_account() {
    ...
    if [[ "$result" == "Success"* ]]; then
        ...
        if [ -n "$user_line" ]; then
            ...
            echo "â™¨áµ—áµ‰Ê³â±áµáµƒáµáµƒË¢â±Ê° áµ—áµ‰Ë¡áµƒÊ° áµáµ‰â¿áµáµáµ˜â¿áµƒáµáµƒâ¿ Ë¡áµƒÊ¸áµƒâ¿áµƒâ¿ áµáµƒáµâ±â™¨"
        fi
    else
...
echo "âœ… Instalasi selesai. Service ZiVPN harusnya aktif dan panel bisa mendeteksi."
    ...
    wget -O /usr/local/bin/zivpn_helper.sh https://raw.githubusercontent.com/suharaaja484/udp-z/main/zivpn_helper.sh
    if [ $? -ne 0 ]; then
